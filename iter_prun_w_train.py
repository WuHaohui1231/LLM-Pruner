...
def main(args):
    ...
    ### EXAMPLES FOR ESTIMATING IMPORTANCE
    example_prompts = get_examples(args.dataset, tokenizer, args.num_examples, seq_len=64)
    print('EG_SHAPE', example_prompts.shape)

    model = model.half()

    def prun_iter(prun_iter_num):

        nonlocal example_prompts
        nonlocal model

        logger.log(str(prun_iter_num) + " PRUNING ITER")

        model.to(args.train_device)
        model.zero_grad()

        optimizer = SGD(model.parameters(), lr=0.00005)
        #############
        # Train the model
        ...
        # path = 'imp_heat_s' + str(args.seed) + '_' + str(args.num_examples) + '_iter1_' + args.dataset[:3] + 'wtr'
    ...
    # ####################
    # print("SECOND PRUNING ITER")
    #
    # ####
    # # Train model for restimate performance
    #
    #
    # ######
    # # CHANGE DEVICE
    ...
    # print("End training")
    #
    # second_device = torch.device(args.sec_device)
    #
    # model = model.to(second_device)
    #
    # example_prompts = example_prompts.to(second_device)
    #
    # pruner_type = args.pruner_type.lower()
    # assert pruner_type in ['random', 'l2', 'l1', 'taylor']
    #
    ...
    #     [1, 3439, 17632, 1925, 29892, 278, 6368, 310],
    # ]).to(
    #     second_device)  # Only for building the dependency graph. Any input will be fine since the computation result are not taken into consideration.
    #
    # if pruner_type == 'random':
    #     imp = tp.importance.RandomImportance()
    # elif pruner_type == 'l1':
    ...
    #
    # path = 'imp_heat_s' + str(args.seed) + '_' + str(args.num_examples) + '_iter2_' + args.dataset[:3] + 'wtr'
    # pruner.visualize_importance(path)
    #
    # del pruner
    #
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sec_device', type=str, default="cuda:7", help='2nd device')
